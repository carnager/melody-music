...
- melodyd: add Last.fm account/scrobbling and love actions, a persistent Up Next request queue, writeback of external queue edits to the active playlist, and a covering tag index.
- melody-musiclist: require an explicit upload host and directory; generated configuration no longer contains a personal destination.

- melody-cli: add Last.fm account setup, scrobbling controls, and love/unlove/info for the current or a named track; add playback context, scratch playlist, batch playlist append, and revision-guarded Up Next commands. Search commands now accept filter expressions, sorting, and windows, including album searches. Shell arguments retain spaces and quotes for ordinary MPD commands; `raw` remains literal protocol text. Help works without a server connection, and lyrics can target an explicit URI.

- melodyd: playback contexts — stored playlists are now independently playable. `melody_context play "Road trip"` materializes a playlist into the queue and starts it, stashing the queue it displaced (songs, priorities, position, elapsed) and remembering where each playlist was left, so switching between lists resumes each one instead of destroying the previous. `melody_context queue` switches back to the stashed queue at its position, preserving the pause state; `melody_context queueinfo` lists it so clients can show the queue while a playlist plays; a bare `melody_context` reports the active one. Editing the active playlist with playlistadd/playlistdelete/playlistmove re-materializes it live, keeping the playing track playing, and rename/rm/clear are handled. State persists across restarts. `melody_context tracks {POS} {URI...}` plays an arbitrary list — a client's scratch tab — the same way, so it no longer has to destroy the queue. Everything runs over the single MPD queue — stock clients just see ordinary queue replacements, with a Melody-only `context` idle subsystem alongside the usual `playlist`/`player` events. Specified in docs/protocol.md.

- melodyd: editable stored playlists — the standard MPD commands `listplaylist` (URI-only listing), `rename`, `playlistdelete`, `playlistmove`, and `playlistclear` are now implemented, `playlistadd` accepts MPD 0.23's optional insert position, and `save` over an existing name replaces its content instead of silently accumulating duplicate playlist rows. Positional edits address entries (not track ids), so playlists containing the same track twice edit correctly, and positions stay contiguous after deletions. This lights up the playlist remove/reorder/rename UI that MPD clients gate on these commands — Trackknife's stored-playlist tabs among them — with no client changes.

- melodyd: guided first-run setup — starting `melodyd` in a terminal without a usable configuration now walks through an interactive setup (music folder with `~` expansion and existence checks, MPD port, HTTP address, optional web password, server name) and writes `melodyd.toml` before starting; the new `melodyd setup` command re-runs it any time with the current settings as defaults, keeping hand-added keys and backing the old file up to `melodyd.toml.bak`. `melodyd version` and `melodyd help` exist now too. Headless starts (systemd) no longer die cryptically or restart-loop: a missing or incomplete config writes the skeleton, logs exactly what to do, and exits with a status the bundled service file recognizes as "do not restart" (`RestartPreventExitStatus=78`). An inaccessible `music_dir` is reported at startup the same way, unless `required_mounts` says to wait for the share.

- melodyd: full filter grammar — `find`/`search`/`findadd`/`searchadd`/`searchalbums` now parse MPD 0.21+ filter expressions completely instead of a flat AND-only subset: arbitrary nesting, `!` negation, the `!=` operator, and the empty-value present/missing forms (`(Genre == '')` matches songs without the tag, `(Genre != '')` songs with it) all work per stock MPD, numeric comparisons apply to any tag by leading integer (`find "((date > 1995) AND (date < 2010))"`), and `OR` disjunction is available as a Melody extension (`find "((Genre == 'Jazz') OR (Genre == 'Blues'))"`; gate on the new capability marker since stock MPD has no `OR`). The new `filtergrammar` command advertises the level (`grammar: 2`), flat conjunctions keep their indexed fast paths, structured trees evaluate per track with sort/window intact, and malformed expressions return an `ACK` instead of a silently empty result. Specified in docs/protocol.md.

- melodyd: stream technicals — the scanner now reads codec, sample rate, bit depth, and channel count from the container headers it already parses (FLAC STREAMINFO, Ogg Vorbis/Opus identification, MP3 frame headers, MP4 sample entries) and stores them per track; existing libraries backfill on the next ordinary scan without touching file mtimes. Song listings gain the standard `Format: rate:bits:channels` line (with `f` for lossy codecs) plus `X-Codec`, and `find`/`search` accept `samplerate`, `bitspersample`, `channels`, `length`, and `codec` filter conditions (`find "(samplerate >= 96000)"`, `find "(codec == 'flac')" "(Genre == 'Jazz')"`), which also work as track-level terms in `searchalbums` — `searchalbums "(samplerate >= 96000)"` lists your hi-res albums.

- melodyd: album-shaped search — the new `searchalbums` command (advertised in `commands`, specified in docs/protocol.md) answers filter expressions with one record per album instead of the protocol's usual song list: `AlbumArtist`/`Album`/`Date` plus `X-AlbumId`, `X-TrackCount`, `X-Duration`, the stored `X-Rating`, the `X-ComputedRating` track mean (at the 70% threshold), and an `X-ArtworkUri` usable with `albumart`/`readpicture`. Album-level terms (`albumartist`, `album`, `date`, `albumrating`, `added-since`) apply to the album directly; track-level terms (`rating`, `artist`, `title`, `any`, generic tags) match albums containing at least one matching track; `sort [-]albumartist|album|date|added|rating` and `window` are supported, and unsupported terms return an error instead of silently broadening the result. `searchalbums "(albumrating >= 8)" sort -rating` is "my best albums" in one round trip.

- melodyd: generic tag support — the scanner now extracts all recognized file tags into the database and exposes them as first-class MPD tags. This covers Genre (multi-valued), Composer, Performer, Conductor, Ensemble, Work, Grouping, Mood, Label, Comment, OriginalDate, the sort tags (ArtistSort, AlbumArtistSort, AlbumSort, TitleSort), and the full MusicBrainz ID set (`MUSICBRAINZ_ARTISTID`, `MUSICBRAINZ_ALBUMID`, `MUSICBRAINZ_ALBUMARTISTID`, `MUSICBRAINZ_TRACKID`, `MUSICBRAINZ_RELEASETRACKID`, `MUSICBRAINZ_RELEASEGROUPID`, `MUSICBRAINZ_WORKID`). Raw keys are normalized across formats (Vorbis/FLAC comments, ID3v2 frames including TXXX and the Picard UFID recording ID, MP4 iTunes freeform atoms). All of them appear in `tagtypes` and song listings, work in `find`/`search` filters (`find "(Genre == 'Shoegaze')"`, `find "(MUSICBRAINZ_TRACKID == '...')"`) including combinations with artist/album/date, and can be enumerated with `list Genre` (optionally filtered, e.g. `list Genre AlbumArtist "X"`) and browsed with `list Album Genre "X"`. Generic-tag listings support `group` on album/albumartist/artist/date (`list musicbrainz_releasegroupid group album group albumartist` emits one AlbumArtist/Album/MBID row per album); an unsupported group tag returns an error instead of silently degrading to a flat list. On first start after the upgrade the library's file metadata is re-read once to populate the new tags.
