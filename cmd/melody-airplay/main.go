...
type agentConfig struct {
	...
	AirPlay struct {
		Device string `toml:"device"` // CoreAudio output device name, optional
	} `toml:"airplay"`
}
...
func main() {
	logger := log.New(os.Stdout, "melody-airplay: ", log.LstdFlags)
	...
	if cfg.AirPlay.Device != "" {
		if err := selectOutputDevice(cfg.AirPlay.Device); err != nil {
			logger.Fatalf("select airplay output %q: %v", cfg.AirPlay.Device, err)
		}
		logger.Printf("selected audio output: %s", cfg.AirPlay.Device)
	} else {
		logger.Printf("no airplay.device configured; using current macOS default audio output")
	}
	...
	logger.Printf("airplay agent %q connecting to master at %s", cfg.Agent.Name, cfg.Agent.Master)
...
func loadAgentConfig() (agentConfig, error) {
	...
	configPath := filepath.Join(xdgConfig, "melody", "melody-airplay.toml")
	...
	if cfg.Agent.Name == "" {
		...
		cfg.Agent.Name = hostname + "-airplay"
	}
	if cfg.Agent.Master == "" {
...
[airplay]
# Optional CoreAudio output device name. On macOS, AirPlay speakers selected
# in Control Center usually appear as normal output devices.
# Leave empty to use the current default output.
device = ""
