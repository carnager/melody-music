# Playback contexts — implementation plan (agent handoff)

This plan spans TWO repositories: Part A is this repository (melodyd) and
ships/deploys first; Part B lives in ../trackknife (its copy is in
CODEX_TASKS.md there). Keep each repo green independently. The hard
constraint throughout: melodyd stays 100% MPD compatible — no stock command
changes semantics, no new status keys; the extension surface is exactly the
advertised melody_context command plus the context idle subsystem.
