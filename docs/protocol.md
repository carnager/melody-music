...
melody_scratch {NAME} {0|1}        -> OK
```

A client's working tab is a stored playlist like any other — same commands,
same contexts, visible to every client — flagged so clients can present it as
a working list instead of listing it beside curated playlists. Clearing the
flag promotes the list; deleting the playlist removes it. Lists live on the
